...
/* === Mobile fixes for VibeMatch (paste at end) === */

/* 1) Reduce top whitespace + center header nicely */
@media (max-width: 430px) {
  .quiz-section{
    /* less padding at top, keep safe-area at bottom */
    padding: 0.8rem 0.9rem calc(1.1rem + env(safe-area-inset-bottom));
    gap: .4rem;
  }
  ...
  .quiz-header{
    ...
    text-align: center;
    gap: .3rem;
    max-width: 36rem;     /* prevents super-wide subtitle lines */
  }
  ...
  .progress-text{ font-size: .85rem; }
  .progress-dots{ gap: .32rem; }
  .dot{ width: 9px; height: 9px; }

  /* Pull the card up and make it fit the viewport width cleanly */
  .quiz-card{
    margin-top: .2rem;
    padding: .95rem;
  ...
  .step{ gap: .7rem; }
  .step-title{ font-size: 1.15rem; margin: .15rem 0 .45rem; }
}

/* 2) Fix the “Next” button clipping beside the input on iPhone */
.location-input-wrap{
  display: grid;
  grid-template-columns: 1fr auto; /* input grows, button keeps its size */
  align-items: center;
  gap: .5rem;
}
.location-input{ padding: .7rem .95rem; font-size: 16px; } /* 16px avoids iOS zoom */
.location-step .btn{
  padding: .7rem 1rem;
  min-width: 96px;         /* gives the button a predictable width */
  max-width: 120px;
}

/* 3) On very small phones, stack input and button so nothing overflows */
...
/* 4) Smallest devices: nudge headline a bit smaller */
@media (max-width: 360px){
  .title{ font-size: clamp(1.5rem, 6.8vw, 1.95rem); }
}

/* 5) Keep the card clear of the floating menu button when scrolling-to */
@media (max-width: 430px){
  .quiz-card{ scroll-margin-top: 72px; }
}

/* 6) Smooth scrolling in the suggestions list */
.autocomplete-list{
  max-height: 45vh;
  -webkit-overflow-scrolling: touch;
}

/* 7) Prevent iOS auto-zoom weirdness across the page */
